import { appImages } from "./Images"

export const causeData = [
    {
        causeType: "Urgent Causes",
        causeDataList: [
            {
                img: appImages.urgentCauseImgOne,
                heading: "WOMEN EMPOWERMENT",
                description: "Women empowerment program works towards the betterment and financial capacity building of Women from the slums and underprivileged section of the society. These programs focus on making women financially independent and self-reliant.",
            },
            {  img: appImages.urgentCauseImgTwo,
                heading: "HUNGER AND POVERTY",
                description: "Food donations help feed the poor and hungry. Help us to Send Food by taking part in the Food Donation Drive or by donating to the food kitchens, lunch delivery or tiffin lunch box to feed the needy. Make a small food donation today and help the hungry and poor people.",
            },
            {
                img: appImages.urgentCauseImgThree,
                heading: "EDUCATION AND TRAINING",
                description: "Come, Join Save the Children or Donate to Educate Every Poor Child in India.Over the years, we have faced a multitude of issues but undeterred,we have devised several solutions that help us continue sharing the gift of education with underprivileged children.",
            },
            {
                img: appImages.urgentCauseImgOne,
                heading: "EDUCATION AND TRAINING",
                description: "Come, Join Save the Children or Donate to Educate Every Poor Child in India.Over the years, we have faced a multitude of issues but undeterred,we have devised several solutions that help us continue sharing the gift of education with underprivileged children.",
            },
            {
                img: appImages.urgentCauseImgTwo,
                heading: "Be Inspired by Changemakers",
                description: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatibus culpa expedita saepe ad. Totam repudiandae error ea! Temporibus accusamus veritatis minima, ipsa accusantium at laborum?",
            },
            {
                img: appImages.urgentCauseImgThree,
                heading: "Be Inspired by Changemakers",
                description: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatibus culpa expedita saepe ad. Totam repudiandae error ea! Temporibus accusamus veritatis minima, ipsa accusantium at laborum?",
            },
        ],
        btnType: "Explore More"
    },
    {
        causeType: "Explore Our Recent Causes That We Works",
        causeDataList: [
            {
               img: appImages.recentCauseImgOne,
                heading: "Providing Healthy Food",
                description: "This is done by organizing Food Donation Drive. Community/individual bring food donations to a place, on a specific date, where volunteers are waiting to receive it.",
            },
            {
                img: appImages.recentCauseImgTwo,
                heading: "Clothes Donation Drive",
                description: "Our Clothes donation drive is exclusively meant for the children they donate reusable clothes, shoes, toys, stationery, furniture, electronics to trusted NGOs. Easy, transparent, hassle-free service.",
            },
            {
                img: appImages.recentCauseImgThree,
                heading: "Supply Drinking Water",
                description: "Water For People is a clean water charity that has helped millions of people across nine countries get access to clean water. You can make a difference!",
            },
            {
                img: appImages.recentCauseImgOne,
                heading: "Medical Mission",
                description: "Get medical treatment funding to pay surgery, post-op care and hospital bills by starting a fundraiser in under five minutes. ",
            },
            {
                img: appImages.recentCauseImgTwo,
                heading: "Clothes Donation Drive",
                description: "Our Clothes donation drive is exclusively meant for the children they donate reusable clothes, shoes, toys, stationery, furniture, electronics to trusted NGOs. Easy, transparent, hassle-free service.",
            },
            {
                img: appImages.recentCauseImgThree,
                heading: "Medical Mission",
                description: "Get medical treatment funding to pay surgery, post-op care and hospital bills by starting a fundraiser in under five minutes. ",
            },
        ],
        btnType: "View More"
    }
]